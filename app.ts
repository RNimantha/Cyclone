// Configuration
const API_URL: string = '/api/donations';
const REFRESH_INTERVAL: number = 30000; // 30 seconds

// Store current donation data for PDF export
let currentDonationData: DonationResponse | null = null;
...
async function fetchDonationData(): Promise<void> {
...
function updateDashboard(totalAmount: number, totalDonors: number): void {
...
function sortDonationsByAmount(donations: ProcessedDonation[], order: 'asc' | 'desc'): ProcessedDonation[] {
...
// PDF Export Function
function downloadDonationPDF(): void {
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(
            `Page ${i} of ${pageCount} - Cyclone Relief Fund Donation Records`,
            105,
            290,
            { align: 'center' }
        );
